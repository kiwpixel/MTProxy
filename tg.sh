#!/bin/bash
# MTProto 一键安装脚本 (带AdTag配置)
# Author: HgTrojan (modified version)
...
export MTG_ADTAG="$MTG_CONFIG/adtag"
...
getData() {
    ...
    # 新增AdTag配置
    colorEcho $BLUE "\n ${INFO} AdTag 用于在Telegram @MTProxybot注册后获取的标签"
    read -p " 请输入 AdTag (留空则不设置): " ADTAG
    if [[ -n "$ADTAG" ]]; then
        echo "$ADTAG" > "$MTG_ADTAG"
    else
        rm -f "$MTG_ADTAG" 2>/dev/null
    fi
...
start() {
    ...
    if [[ -f "$MTG_ADTAG" ]]; then
        ADTAG=$(cat "$MTG_ADTAG")
        if [[ -n "$ADTAG" ]]; then
            ADTAG_OPTION="--adtag $ADTAG"
            colorEcho $BLUE " ${INFO} 已应用 AdTag 配置"
        fi
    fi
    ...
    if ! docker run -d \
        ...
        "$MTG_IMAGENAME" run $ADTAG_OPTION "$(cat "$MTG_SECRET")" >/dev/null; then
...
showInfo() {
    SECRET=$(cat "$MTG_SECRET" 2>/dev/null) || SECRET="未生成"
    ADTAG=$(cat "$MTG_ADTAG" 2>/dev/null) || ADTAG="未设置"
    set -a; source "$MTG_ENV"; set +a
    SUBSCRIPTION_LINK=$(generateSubscriptionLink)
    ...
    echo -e " ${BLUE}● AdTag:${PLAIN} ${GREEN}$ADTAG${PLAIN}"
...
menu() {
    ...
    echo -e "#           ${YELLOW}支持 AdTag 配置 | 作者: HgTrojan${PLAIN}        #"
    echo -e "#===============================================#${PLAIN}"
    ...
    echo -e "  ${GREEN}8.${PLAIN} 修改配置 (包括AdTag)"
